...
export async function PATCH(request: Request) {
  try {
    ...
    const queries = sellerIds.map(id => ({
      query: `UPDATE sellers SET ${field} = ?, updated_at = ? WHERE seller_id = ?`,
      params: [value, new Date(), cassandraClient.types.Uuid.fromString(id)]
    }));
  ...
  try {
    ...
    const allQueries = sellerIds.flatMap(id => 
      tables.map(table => ({
        ...
        params: [cassandraClient.types.Uuid.fromString(id)]
      }))
    );
